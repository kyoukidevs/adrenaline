...
local TEAM_OFFSET = Vector2.new(0, 2);
...
function EspObject:Construct()
	...
	self.drawings = {
		...
		visible = {
			...
			team = self:_create("Text", { Center = true, Visible = false }), -- Добавлено: текст команды
...
function EspObject:Update()
	...
	self.teamName = interface.getTeamName(self.player); -- Добавлено: получение названия команды
...
function EspObject:Render()
	...
	-- Добавлено: отображение названия команды
	visible.team.Visible = enabled and onScreen and self.teamName and options.team;
	if visible.team.Visible then
		local team = visible.team;
		team.Text = self.teamName;
		team.Size = interface.sharedSettings.textSize;
		team.Font = interface.sharedSettings.textFont;
		team.Color = parseColor(self, options.teamColor[1]);
		team.Transparency = options.teamColor[2];
		team.Outline = options.teamOutline;
		team.OutlineColor = parseColor(self, options.teamOutlineColor, true);
		
		-- Позиционирование под именем игрока
		local namePosition = visible.name.Visible and (visible.name.Position + Vector2.new(0, visible.name.TextBounds.Y)) or
							(corners.topLeft + corners.topRight)*0.5;
		team.Position = namePosition + TEAM_OFFSET;
	end
...
local EspInterface = {
	...
	teamSettings = {
		enemy = {
			...
			team = true, -- Добавлено: отображение команды
			teamColor = { "Team Color", 1 }, -- Использует цвет команды
			teamOutline = true,
			teamOutlineColor = Color3.new(),
			weapon = false,
		...
		friendly = {
			...
			nameOutline = true,
			nameOutlineColor = Color3.new(),
			team = false, -- Добавлено: отображение команды
			teamColor = { "Team Color", 1 }, -- Использует цвет команды
			teamOutline = true,
			teamOutlineColor = Color3.new(),
			weapon = false,
...
-- Добавлено: получение названия команды
function EspInterface.getTeamName(player)
